        ...
        <div class="flex-none w-full max-w-full px-3">
            <div class="relative flex flex-col min-w-0 mb-6 break-words bg-white border-0 border-transparent border-solid shadow-soft-xl rounded-2xl bg-clip-border">
                ...
                <div class="p-6 pb-0 mb-0 bg-white border-b-0 border-b-solid rounded-t-2xl border-b-transparent flex justify-between items-center">
                    ...
                    {{-- FIX TOMBOL: Paksa Gradient Manual --}}
                        ...
                        <table class="items-center w-full mb-0 align-top border-gray-200 text-slate-500">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    {{-- FIX STATUS: Pake Style Manual (Inline CSS) --}}
                                    <td class="p-2 text-sm leading-normal text-center align-middle bg-transparent border-b whitespace-nowrap shadow-transparent">
                                        @if($project->status == 'Pending')
                                            {{-- Pending: Abu-abu --}}
                                            <span style="background-color: #a0aec0; color: white; padding: 5px 10px; border-radius: 6px; font-size: 11px; font-weight: bold; text-transform: uppercase;">
                                                Pending
                                            </span>
                                        @elseif($project->status == 'In Progress')
                                            {{-- In Progress: Biru Ungu --}}
                                            <span style="background-color: #5e72e4; color: white; padding: 5px 10px; border-radius: 6px; font-size: 11px; font-weight: bold; text-transform: uppercase;">
                                                In Progress
                                            </span>
                                        @elseif($project->status == 'Completed')
                                            {{-- Completed: Hijau --}}
                                            <span style="background-color: #2dce89; color: white; padding: 5px 10px; border-radius: 6px; font-size: 11px; font-weight: bold; text-transform: uppercase;">
                                                Completed
                                            </span>
                                        @else
                                            <span style="background-color: #344767; color: white; padding: 5px 10px; border-radius: 6px; font-size: 11px; font-weight: bold; text-transform: uppercase;">
                                                {{ $project->status }}
                                            </span>
                                        @endif
                                    </td>
